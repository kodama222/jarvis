...
def load_data():
    ...
    token_final_dist = ape.drop(
        columns=['Total', 'New Supply', 'Annual Inflation']
    ).iloc[-1]
...
def main():
    ...
    if option == 'All Holders':
        ...
        fig = go.Figure(
            data=[
                go.Pie(
                    labels=token_final_dist.index,
                    values=token_final_dist.values,
                )
            ]
        )
        fig.update_traces(
        ...
        fig = px.area(
            ape.drop(columns=['Total', 'New Supply', 'Annual Inflation']),
        ...
        fig = go.Figure(
            data=[
                go.Pie(
                    labels=in_nonin_final_dist.index,
                    values=in_nonin_final_dist.values,
                )
            ]
        )
        fig.update_traces(
    ...
    fig.add_trace(
        go.Bar(x=ape.index, y=ape['New Supply'], name='New Monthly Supply')
    )
    fig.add_trace(
        go.Scatter(
            x=ape.index,
            y=ape['New Supply'].cumsum(),
